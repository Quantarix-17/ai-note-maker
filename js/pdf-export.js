  ...
  return `<!DOCTYPE html><html><head>
    ...
    <style>
      ...
      * { box-sizing: border-box; -webkit-print-color-adjust: exact !important; print-color-adjust: exact !important; color-adjust: exact !important; }
      ...
      ${isMonochromeMode ? `
      /* PHOTOCOPY-STANDARD callout boxes. The colored-theme version tells
         each callout type apart by background tint and border color alone
         — with no color available, that distinction disappears entirely
         and every box would look identical. Real black-and-white textbooks
         solve this with typography instead of color: a plain white box, a
         firm black rule, and the bold label the AI already writes inside
         each callout ("Definition:", "Example:", ...) doing the work color
         used to do. One consistent box style reads as deliberate,
         photocopy-friendly design — not a colored theme with the color
         switched off. */
      .block-example,.block-definition,.block-warning,.block-important,.block-note{background:#fff;border:1pt solid #000;border-left:3.5pt solid #000;padding:10px 14px;margin:10px 0;border-radius:0}
      .block-accent{background:transparent !important;border:none !important;border-left:3.5pt solid #000 !important;padding:8px 14px;margin:10px 0}.block-solution{background:#fff;border:1pt solid #000;border-radius:0;padding:14px 16px 10px;margin:14px 0}
      ` : `
