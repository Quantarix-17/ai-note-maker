...
function renderAtCommandMenuList() {
  ...
  commandTitle.className = 'at-command-section-title';
  ...
  AT_MENU_STATE.filtered.forEach((cmd, idx) => {
    ...
    commandSection.appendChild(item);
  });
  ...
  [
    ...
    sw.textContent = f.id === 'default' ? 'Aa' : f.id === 'english' ? 'EN' : f.id === 'english_bengali' ? 'EN/BN' : 'BN/EN';
    const info = document.createElement('span');
    info.className = 'at-cmd-text';
    const label = document.createElement('span');
    label.className = 'at-cmd-label';
