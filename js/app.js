...
function buildSharedRules(isMonochromeMode, outputLanguage) {
  ...
  const languageFormatInstruction = languageFormat === 'english' ? `
    === LANGUAGE FORMAT ===
    Write the document in English only. Do not add Bengali translations.
  ` : languageFormat === 'english_bengali' ? `
    === LANGUAGE FORMAT ===
    Present the document bilingually: write each section in English first, followed immediately by its Bengali translation. Keep the same structure and meaning in both languages.
  ` : languageFormat === 'bengali_english' ? `
    === LANGUAGE FORMAT ===
    Present the document bilingually: write each section in Bengali first, followed immediately by its English translation. Keep the same structure and meaning in both languages.
  ` : '';
...
window.onload = function() {
  ...
  if (typeof switchPreviewTabDesktop === 'function') {
    switchPreviewTabDesktop((typeof isMobileDeviceLayout === 'function' && isMobileDeviceLayout()) ? 'chat' : 'chat');
  }
